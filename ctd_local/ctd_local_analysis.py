...
def run_psd_beta(df_anom: pd.DataFrame, anom_cols: list[str], fs: float, max_nperseg: int, figures_dir: Path, metrics_dir: Path):
    ...
    fig, axes = plt.subplots(n_vars, 1, figsize=(10, max(3 * n_vars, 6)))
...
def run_pca_alpha(df_anom: pd.DataFrame, var_list: list[str], group_name: str, figures_dir: Path):
    ...
    if valid.sum() < 2:
        summary = pd.DataFrame([{"group": group_name, "alpha": np.nan, "r2": np.nan, "n_obs": X.shape[0], "n_vars": X.shape[1], "interpretation": "insufficient positive eigenvalues"}])
...
def run_pipeline(config_path: str | Path):
    ...
    print("Análisis CTD local terminado.")
    print(f"Directorio de salida: {output_dir}")


if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Análisis posterior para CTD local.")
