...
def _build_predictor_set(df: pd.DataFrame, target_col: str, predictor_cols: list[str], time_col: str):
    ...
    time_feats = [
        "hour", "minute", "dayofyear", "hour_sin", "hour_cos", "minute_sin", "minute_cos", "doy_sin", "doy_cos"
    ]
    cols = []
    seen = set()
...
def rf_gapfill_variable(df: pd.DataFrame, target_col: str, predictor_cols: list[str], time_col: str, rf_params: dict):
    ...
    if len(pred_df) > 0:
        out.loc[pred_df.index, target_col] = model.predict(pred_df[X_cols])
...
def hybrid_gapfill_segment(
    ...
    for start, end in remaining_runs:
        if (end - start + 1) <= max_rf_gap_steps:
            rf_candidate_idx.extend(range(start, end + 1))

    if rf_candidate_idx:
        rf_out, _ = rf_gapfill_variable(out, target_col, predictor_cols, time_col, rf_params)
    ...
    if still_missing.any():
        rf_time_out, _ = rf_gapfill_time_only(out, target_col, time_col, rf_params)
...
def validate_by_masking_hybrid(
    ...
    for idx in chosen:
        ...
        pred_df, _ = hybrid_gapfill_segment(
            df_segment=sub,
...
def run_pipeline(config_path: str | Path):
    ...
    analysis_ready_dir = ensure_dir(output_dir / "analysis_ready_segments")
    ...
    df_regularized.to_csv(output_dir / "ctd_local_regularized.csv", index=False)
    segment_summary.to_csv(output_dir / "ctd_local_segment_summary.csv", index=False)
    df_gapfilled_all.to_csv(output_dir / "ctd_local_gapfilled_all_segments.csv", index=False)
    df_gapfill_flags.to_csv(output_dir / "ctd_local_gapfill_flags.csv", index=False)
    df_validation.to_csv(output_dir / "ctd_local_gapfill_validation_summary.csv", index=False)
    df_method_counts.to_csv(output_dir / "ctd_local_gapfill_method_counts.csv", index=False)
    summary.to_csv(output_dir / "ctd_local_gapfill_summary_combined.csv", index=False)
    ...
    print("Gap filling CTD local terminado.")
    print(f"Directorio de salida: {output_dir}")
    print(f"Segmentos detectados: {segment_summary['segment_id'].astype(int).tolist()}")


if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Gap filling híbrido para CTD local.")
